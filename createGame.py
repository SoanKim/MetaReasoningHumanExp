    ...
    def genLeafVal(self):
        ...
        * dims:                                                    * self.leafVal:
        ...
        for prb_i, (stim, ans) in enumerate(self.env):
            ...
            for dim in range(4):
                for elem in range(3):
                    self.leafVal[prb_i, elem, dim] = 0 if len(dims[dim][elem]) == 0 else np.round(1/len(dims[dim][elem]), 2)
...
# makeTree().initState()
    ...
    # def genAction(self):
    #     """
    #     self.current == 0 -> Default
    #     self.current == 1 -> Env
    #     self.current == 2 -> Same / Err / Diff
    #     self.current == 3 -> Triplet candidates
    #     """
    #     if self.current == 0:
    #         actions = None
    #     elif self.current == 1:
