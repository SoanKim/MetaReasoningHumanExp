...
class MCTS:
        ...
        if self.depth == 0:
            ...
            while self.getBestRoot() is not None:
                ...
                if len(self.getState(node)[0]) == 0:
                    ...
                    print("best child.current", bestChild.current)
                    finalReward = self.leafState[bestChild.current[0]*4+bestChild.current[1]-1]
                    # finalReward = self.selectCard(bestChild)
    ...
    def getBestChild(self, node):
        ...
        #print(f"maxUCB & maxUCB position: {maxUCB},{maxUCBposition}")
    ...
    def selectCard(self, bestChild):
        ...
        if len(cardsAvail) == 0:
            ...
            if self.prbIdx < 60:
                ...
                reward = self.getLeafVal(bestChild.current)
            self.updateQ(bestChild.current, reward)
        # self.backprop(reward=reward, node=bestChild)
        return reward
